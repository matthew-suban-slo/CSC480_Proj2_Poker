...
# Map rank characters to integers
...
def parse_card(card_str):
    """
    Parse a card string into (rank_int, suit_char).
    e.g. 'AH' -> (14, 'H')
    """
...
def evaluate_5cards(cards):
    """
    Evaluate a 5-card poker hand and return a tuple for comparison.
    Category (9=straight flush,...,1=high card) plus tiebreakers.
    """
...
def best_hand_score(seven_cards):
    """Return the best 5-card score from 7 cards."""
    best = None
...
def evaluate_win(hole, opp_hole, board):
    """Return 1 for win, 0.5 for tie, 0 for loss."""
...
def simulate(hole_cards, simulations=20000, time_limit=10.0):
    """
    Run MCTS with UCB1 over opponent hole combos and random full board draws.
    Print win/tie/loss rates, counts of stays vs. folds, and decision.
    """
